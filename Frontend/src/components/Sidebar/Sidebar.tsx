...
export function Sidebar({ setCurrent }: { setCurrent: Dispatch<SetStateAction<Chat | null>> }) {
	...
	return (
		<aside className="
			fixed right-0 top-0 
			w-100 h-screen 
			bg-gradient-to-b from-[#052a3b] to-black-800
			text-white
			p-4
			shadow-xl
			overflow-hidden
		...
		">
			...
			{/* Perfil de usuario */}
			<div className="flex justify-between items-center top-0 left-0 right-0 p-2 rounded-lg bg-gray-600/40">
				<div className="flex items-center space-x-3">
					<Sheet>
						...
						<SheetContent 
						aria-describedby=""
						style={{ width: '500px', maxWidth: '90vw', background: 'linear-gradient(to bottom, #000000 0%, #080f1f 50%, #02354b 100%)' }}
						side="left"
						>
							<SheetHeader>
			...
			<div className="mb-5 mt-5">
				<h2 className="text-2xl text-center font-bold mb-2">Chats</h2>
			</div>
				...
				{chats.length !== 0 && chats.map((chat) => {
					return (
						<div key={chat.id} className="grid gap-y-5 p-1">
							<div className="group relative bg-[#77777745] rounded-lg gap-y-2 p-3 max-h-[80px] hover:bg-[#40454b60] transition-all animation-all cursor-pointer">
									...
									<div className="flex w-80">
										<div className="flex  justify-between text-zinc-400 p-2">
											...
											{!chat.lastMessage.readed &&
												<div className="rounded-full absolute right-10 top-1/2 bg-[#649ba7c5] text-white flex justify-center items-center text-[12px] p-1">
				...
				<Dialog open={openCreate} onOpenChange={setOpenCreate} >
					<DialogTrigger asChild>
						<Tooltip title="Comenzar nuevo chat" placement="left">
							<IconButton aria-label="Add new chat" size="large" color="info" >
